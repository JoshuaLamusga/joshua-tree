...
export function parseStory(
  story: string,
  interpreter: React.RefObject<StoryInterpreterC>,
  forkToLoad?: string
) {
    ...
    if (newlineSplitStory[entryPositions[i]].length < 2) {
      if (interpreter.current) {
        interpreter.current.setErrorMessage( 
          "Parser: Entry" +
            newlineSplitStory[entryPositions[i]] +
      ...
      if (elemType === 0) {
        ...
        if (j !== 0 && prevIfBegin !== -1) {
          ...
          if (elemBegin - prevElemEnd < 0) {
            if (interpreter.current) {
              interpreter.current.setErrorMessage( 
                `Parser: In '${text.substring(prevElemBegin)}'` +
                  ", cannot specify multiple if tokens on one line.")
            }
      ...
      // Handles endif keywords.
        ...
        if (elemBegin - prevElemEnd < 0) {
          if (interpreter.current) {
            interpreter.current.setErrorMessage( 
              "Parser: In '" +
                text.substring(prevElemBegin) +
                "', cannot specify multiple endif tokens on one line.")
          }
