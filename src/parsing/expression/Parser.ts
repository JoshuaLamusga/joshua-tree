...
export class Parser {
  ...
  public Div = new TokenOp(placements.both, associativity.left, 7, "/", (operands: IToken[]) => {
    if (operands[0] instanceof TokenNum && operands[1] instanceof TokenNum) {
      ...
      if (n1.value === 0) {
        throw new Error(
          `Parser: The expression ${n0.strForm} / ${n1.strForm} causes division by zero.`
        );
      }
  ...
  public evalTokens(tokensList: IToken[]) {
    ...
    for (let i = 0; i < tokensList.length; i++) {
      if (tokensList[i] instanceof TokenId) {
        ...
        // Replaces unknown identifiers with a value or fails.
          ...
          throw new Error(
            "Parser: The identifier '" +
              tokensList[i].strForm +
              "' is unknown and can't be computed."
          );
        }
      }
    }
    ...
    while (true) {
      ...
      if (expressionLHS[expressionLHS.length - 1] instanceof TokenFunc) {
        ...
        subExpressionFunc =
          functions.find((f) => f.numArgs === numArgs && f.strForm === tokFunc.strForm) ?? null;
      }
  ...
  public evalNoPar = (subExpression: IToken[], func: TokenFunc | null): IToken[] => {
    let identifiers = this.tokens.filter((tok) => tok instanceof TokenId) as TokenId[];
    ...
    if (
      ...
      throw new Error(
        "Parser: The expression '" +
          subExpressionStr +
          "' is missing a right parenthesis at the end."
      );
    }
    ...
    if (subExpression.length === 0) {
      throw new Error(
        "Parser: an empty parenthesis group was provided; there is nothing to process within it."
      );
    }
    ...
    if (func !== null) {
      ...
      if (immediateResult === null) {
        throw new Error(
          "Parser: In expression '" +
            subExpressionStr +
            "', arguments do not match parameter types used."
        );
      }
    ...
    while (maxPrecedence > 0) {
      let isRightAssociative = operators.some(
        (tok) => maxPrecedence === tok.prec && tok.assoc === associativity.right
      );
      ...
      while ((isRightAssociative && j >= 0) || (!isRightAssociative && j < subExpression.length)) {
        if (
          subExpression[j] instanceof TokenOp &&
          ...
          if (
            argVals[0] === null &&
            (opToken.placement === placements.both || opToken.placement === placements.left)
          ) {
            throw new Error(
              "Parser: In '" +
                subExpressionStr +
          ...
          if (result === null) {
            throw new Error(
              "In expression '" + subExpressionStr + "', operand type(s) do not match operator."
            );
          } else {
  ...
  public tokenize(expression: string): IToken[] {
    ...
    for (let i = 0; i < expression.length; i++) {
      ...
      // Matches when there are no longer candidates.
        ...
        longestMatch !== null &&
        (i !== expression.length - 1 || longestMatch.strForm.length === token.length)
      ) {
